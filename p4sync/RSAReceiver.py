...
def handle_pkt(pkt):
    ...
    if Signature in pkt:
        ...
        ensig = str(bin(long(pkt["Signature"].signature))[2:]).zfill(1024)
        # print ensig
        desig = bits2string([ensig[i:i+8] for i in range(0, len(ensig), 8)])
        result = rsa_verify(desig, message.encode('utf-8'), public_key)
